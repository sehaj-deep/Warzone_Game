...
public class GameEngine {
	...
	/**
	 * Setter function to update the current phase of the game
	 * 
	 * @param p_newPhase a new phase representing the current state of the game
	 */
	public void setPhase(Phase p_newPhase) {
		d_currPhase = p_newPhase;
	}
	...
	/**
	 * This function run the game engine to perform behaviors of a phases going
	 * through all phases of the game
	 * 
	 */
	public void start() {
		d_currPhase = new ExecuteOrdersPhase(this);
		d_currPhase.next();
	}
	 ...
	 * 
	 * boolean l_editMapPhase = false; boolean l_gamePhase = false; String
	 * l_userInput = "";
	 * 
	 * while (!l_editMapPhase && !l_gamePhase) {
	 * System.out.println("Do you want to edit the map or start playing the game?");
	 * System.out.println("1. Edit Map"); System.out.println("2. Start Game");
	 * System.out.print("Enter your choice: "); String choice =
	 * l_scanner.nextLine();
	 * 
	 * switch (choice) { case "1": l_editMapPhase = true; break; case "2":
	 * l_gamePhase = true; break; default:
	 * System.out.println("Invalid choice. Please try again."); break; } }
	 * 
	 * // Map editing phase if (l_editMapPhase) {
	 * System.out.println("You are now in the map editing phase."); System.out.
	 * println("To edit the map or create a new map, use the following command:");
	 * System.out.println("editmap filename");
	 * 
	 * l_userInput = l_scanner.nextLine();
	 * 
	 * while (!l_userInput.startsWith("editmap")) { System.out
	 ...
	 * 
	 * System.out.
	 * println("Available commands: editcontinent, editcountry, editneighbor, showmap, savemap, validatemap"
	 * );
	 * 
	 ...
	 * 
	 * if (l_userInput.equalsIgnoreCase("done")) { l_editingMap = false; } else {
	 * d_inputHandler.parseUserCommand(l_userInput); } } }
	 * 
	 * // Game playing phase if (l_gamePhase) { InputHandler l_inputHandlerGame =
	 * new InputHandler(d_mapEditor, d_state);
	 * 
	 * ExecuteOrdersPhase l_executeOrdersPhase = new ExecuteOrdersPhase();
	 * ReinforcePhase l_reinforcementPhase = new ReinforcePhase();
	 * System.out.println("You are now in the game playing phase."); while (true) {
	 ...
	 * l_userInput = l_scanner.nextLine().trim();
	 * l_inputHandlerGame.parseUserCommand(l_userInput); if
	 * (d_mapEditor.getD_countries().size() > 0) { break; } System.out.
	 * println("Map is not loaded. Please load a map before playing a game"); }
	 * initalizeBoard(); // initialize the game board d_state.setGameBoard(d_board);
	 * 
	 * System.out.
	 * println("Available commands: showmap, gameplayer, assigncountries, loadmap");
	 * 
	 * boolean l_playingGame = true;
	 * 
	 * // startup phase: add, remove, and assigncountries while (l_playingGame) {
	 * System.out.print("Enter setup command (type 'assigncountries' to play): ");
	 * l_userInput = l_scanner.nextLine().trim(); if
	 * (l_userInput.equals("assigncountries")) { if (d_state.getPlayers().size() <=
	 * 1) { System.out.println("Require At least 2 players to play a game");
	 * continue; } System.out.println("end start phase"); l_playingGame = false; }
	 * 
	 * l_inputHandlerGame.parseUserCommand(l_userInput);
	 * 
	 * } System.out.println("Game Started"); l_playingGame = true; // game playing
	 * phases: reinforcement, issue order, execute order phases
	 * 
	 * while (l_playingGame) { System.out.
	 * print("Type 'done' to finish, showmap, or enter anything to proceed to issue order: "
	 * ); l_userInput = l_scanner.nextLine().trim();
	 * 
	 * if (l_userInput.equalsIgnoreCase("done")) { l_playingGame = false;
	 * System.out.println("Game Ended"); break; } if (l_userInput.equals("showmap"))
	 * { l_inputHandlerGame.parseUserCommand(l_userInput); }
	 * d_state.setReinforcements(new ArrayList<>());
	 * l_reinforcementPhase.run(d_state, d_mapEditor); // reinforcement phase
