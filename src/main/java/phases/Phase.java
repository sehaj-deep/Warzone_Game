...
//		for (HashMap.Entry<String, Continent> continentEntry : d_continents.entrySet()) {
	 ...
	 * "----------------------------------------------------------------------");
	 * 
	 * HashMap<String, Continent> l_continents = d_gameEngine.getD_continents(); for
	 * (HashMap.Entry<String, Continent> l_cont : l_continents.entrySet()) {
	 * Continent l_currContinent = l_cont.getValue(); String l_nameOfCont =
	 * l_currContinent.getD_continentName();
	 * System.out.println("For the continent: " + l_nameOfCont);
	 * System.out.println("[Country: Neighbors]");
	 * 
	 * Set<Country> l_corresCountries = l_currContinent.getD_countries(); for
	 * (Country l_country : l_corresCountries) { String l_countryName =
	 * l_country.getD_name(); System.out.print(l_countryName + ": ");
	 * 
	 * Set<Country> l_countryNeighbors = l_country.getNeighbors(); for (Country
	 * neighbor : l_countryNeighbors) { String l_neighborName =
	 * neighbor.getD_name(); System.out.print(l_neighborName + " "); }
	 * System.out.println(); } System.out.println(
	 * "----------------------------------------------------------------------"); }
	 * 
	 * if (d_gameEngine.getPhase() == "Preload" || d_gameEngine.getPhase() ==
	 * "PostOrder") { return; } // to get the list of players // FIXME: Find how to
	 * get list of players List<Player> l_allPlayers = p_gameState.getPlayers(); if
	 * (l_allPlayers.size() == 0) { return; }
	 * 
	 * // FIXME: Find how to get list of armies for each country Map<String,
	 * Integer> l_countriesArmies = p_gameState.getGameBoard();
	 * 
	 ...
	 * 
	 * for (String l_singleCountry : l_countriesOwned) { int l_numOfArmies =
	 * l_countriesArmies.get(l_singleCountry); System.out.print("[" +
	 * l_singleCountry + ", " + l_numOfArmies + "] "); } System.out.println(); } }
	 */
}
